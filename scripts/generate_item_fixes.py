...
#
# 「0秒間」は括弧なしの継続時間表記（バンドルパイプ「ファンファーレ」で発覚、
# 2026-08-29）。「10秒間」等の正当な数値の末尾を誤検出しないよう、直前が
# 数字でない場合のみ壊れ判定する。「が/を0増加・0減少」も同じ発覚で追加
# （バフ/デバフの増減量が未解決のケース）。
BROKEN_RE = re.compile(
    r"(を0[獲回]|が0回復|\(0秒\)|ダメージを0|0のダメージ|0の[^。<>]{0,10}?ダメージ|耐久値0|0のシールド|0を獲得"
    r"|(?<!\d)0秒間|が0増加|を0増加|が0減少|を0減少)"
)
...
NEUTRALIZE_SUBS = [
    ...
    (re.compile(r"(?<!\d)0秒間"), ""),
    (re.compile(r"が0増加"), "が増加"),
    (re.compile(r"を0増加"), "を増加"),
    (re.compile(r"が0減少"), "が減少"),
    (re.compile(r"を0減少"), "を減少"),
]
