...
# 古い試合をstateから間引く基準（日）。サンプル数を優先し、多少パッチを
# またいでも許容する方針で約6ヶ月分を保持する（2026-08-19、35日→180日に変更）。
# 大きなリワーク・大幅調整が入った直後は、そのチャンピオンだけ旧パッチの
# データが混ざって数字が鈍る点に留意（新データで薄まるまで数週間かかる）
