...
export function processItemDescription(raw: string): string {
  ...
  s = s.replace(/<gold>/gi, '<span style="color:#E8B34B">').replace(/<\/gold>/gi, '</span>');
  s = s.replace(/<keyword>/gi, '<strong>').replace(/<\/keyword>/gi, '</strong>');
  s = s.replace(/<keywordMajor>/gi, '<strong>').replace(/<\/keywordMajor>/gi, '</strong>');
  s = s.replace(/<rarityLegendary>/gi, '<strong style="color:#E8B34B">').replace(/<\/rarityLegendary>/gi, '</strong>');
  s = s.replace(/<rarityMythic>/gi,    '<strong style="color:#BCA6FF">').replace(/<\/rarityMythic>/gi,    '</strong>');
